...
export default class Question extends React.Component<props, any> {
  createMarkup() {
    return {
      __html: this.props.examQuestion.question.replace(/\n/g, "<br /><br />")
    };
  }
  ...
  public render() {
    ...
    return (
      <div className="text-center">
        {length <= 100 && <h2 dangerouslySetInnerHTML={this.createMarkup()} />}
        {length > 100 && length <= 300 && (
          <h3 dangerouslySetInnerHTML={this.createMarkup()} />
        )}
        {length > 300 && <h4 dangerouslySetInnerHTML={this.createMarkup()} />}
      </div>
    );
  }
}
