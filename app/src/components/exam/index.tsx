...
type props = types.user & types.myCourses & types.message & actionProps;
...
export class Exam extends React.Component<props, state> {
  ...
  next() {
    if (this.state.forwardProgress < 24) {
      ...
      this.props.updateCourseProgress(
        this.state.activeExam,
        this.state.forwardProgress + 1
      );
    } else {
  ...
  public render() {
    const { examContent, forwardProgress, answerCorrect } = this.state;
    ...
    return (
      <div className="text-center">
        ...
        <h3>
          <br />
          <b>{examContent[forwardProgress].module}</b>
        </h3>
        <Question examQuestion={examContent[forwardProgress]} />
        <Helper
